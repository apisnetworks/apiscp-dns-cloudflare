 ...
 */
	...
	class Module extends \Dns_Module implements ProviderInterface
	{
		...
		protected static $permitted_records = [
			'A', 'AAAA', 'CNAME', 'MX', 'LOC', 'SRV',
			'SPF', 'TXT', 'NS', 'CAA', 'PTR', 'CERT',
			'DNSKEY', 'DS', 'NAPTR', 'SMIMEA', 'SSHFP',
			'TLSA', 'URI'
		];
		...
		protected function atomicUpdate(string $zone, Record $old, Record $new): bool
		{
			...
			if (!$this->getRecordId($old)) {
				return error("failed to find record ID in CF zone `%s' - does `%s' (rr: `%s', parameter: `%s') exist?", $zone, $old['name'], $old['rr'], $old['parameter']);
			}
		...
		public function add_record(
			...
			try {
				...
				/** @var $api API */
				$ret = $api->addRecord($this->getZoneId($zone), $record['rr'], $record['name'], $record['parameter'], $record['ttl'], $this->key['proxy'] ?? false, $record->getMeta('priority') ?? '');
